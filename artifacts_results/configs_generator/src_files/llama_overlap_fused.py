...
class LlamaModel(nn.Module):
    ...
    def __init__(self,
        ...
        try:
            self.MAX_CTAS_ATTN = int(os.getenv("MAX_CTAS_ATTN", "16"))
        except ValueError:
            self.MAX_CTAS_ATTN = 16

        try:
            self.MAX_CTAS_MLP = int(os.getenv("MAX_CTAS_MLP", "16"))
        except ValueError:
        ...
        # Attention METADATA
        NUM_SPLITS = 2
        self.split_query_first_loc = torch.zeros(NUM_SPLITS, dtype=torch.int32, device="cuda")
        self.split_query_second_loc = torch.zeros(NUM_SPLITS, dtype=torch.int32, device="cuda")
        self.split_seq_first_loc = torch.zeros(1, dtype=torch.int32, device="cuda")
        self.split_seq_second_loc = torch.zeros(1, dtype=torch.int32, device="cuda")
        ## --------- TokenWeave: pl_overlap_fused --------- ##
    ...
    def forward(
        ...
        # is_tokenweave = False # TODO: remove this line
        ...
        #     print(f"[INFO] tokenweave LlamaModel: {num_tokens=}  {nearest_multiple_of_256=} {tokenweave_chunk_size=}, {self.MAX_CTAS_ATTN=}, {self.MAX_CTAS_MLP=}", flush=True)
