...
def filter_and_aggregate(data, model_name, gpu_count):
    ...
    for batch_size in batch_sizes:
        subset = filtered[filtered['batch_size'] == batch_size]
...
def plot_graphs(data, gpu_count, models, outdir):
    ...
    ax.set_xlabel("Batch Size (Seq Len=4096)", fontweight='bold', fontsize=20)
    ax.set_ylabel('Time (ms)', fontsize=20, fontweight='bold')

    pdf_path = os.path.join(outdir, f'figure_qwen3moe_multibatch.pdf')
...
def main():
    ...
    outdir = sys.argv[2]
    csvdir = sys.argv[3]
    data_path = os.path.join(indir, 'figure_qwen3moe_multibatch_summary.csv')
    data = pd.read_csv(data_path)

    os.makedirs(outdir, exist_ok=True)

    plot_graphs(data, 8, ["Qwen3-235B-A22B"], outdir)
