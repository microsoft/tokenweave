...
def plot_graphs(data, gpu_count, outdir):
    ...
    pdf_path = os.path.join(outdir, 'figure_9.pdf')
...
def main():
    ...
    outdir = sys.argv[2]
    csvdir = sys.argv[3]
    data_path = os.path.join(indir, 'figure_9_summary.csv')
    ...
    combined_df.to_csv(os.path.join(csvdir, 'figure_9.csv'), index=False)
