...
class FusedMoE(torch.nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor,
        ...
        if self.use_direct_call:
            return self.forward_impl(hidden_states, router_logits, is_tokenweave)
        else:
            return torch.ops.vllm.moe_forward(hidden_states, router_logits,
                                              self.layer_name, is_tokenweave)
    ...
    def forward_impl(self, hidden_states: torch.Tensor,
        ...
        # print(f"{self.dp_size=}, {self.ep_size=}, {self.tp_size=}")
        if self.dp_size > 1:
            start = 0 if self.dp_rank == 0 else cu_tokens_across_dp_cpu[
                self.dp_rank - 1]
            end = cu_tokens_across_dp_cpu[self.dp_rank]
        ...
        if self.reduce_results and (self.tp_size > 1 or self.ep_size > 1) and not is_tokenweave:  # noqa: E501
