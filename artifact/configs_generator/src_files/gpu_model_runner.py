...
from vllm.distributed.parallel_state import get_pp_group, graph_capture, get_tensor_model_parallel_rank
...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    def __init__(
        ...
        self.query_first_loc_cpu = torch.zeros(self.max_num_reqs + 1,
                                               ...
                                               device="cpu",
                                               pin_memory=self.pin_memory)
        self.query_first_loc_np = self.query_first_loc_cpu.numpy()
        self.query_second_loc_cpu = torch.zeros(self.max_num_reqs + 1,
                                                dtype=torch.int32,
                                                device="cpu",
                                                pin_memory=self.pin_memory)
        self.query_second_loc_np = self.query_second_loc_cpu.numpy()

        self.seq_lens_first_cpu = torch.zeros(self.max_num_reqs,
                                              dtype=torch.int32,
                                              device="cpu",
                                              pin_memory=self.pin_memory)
        self.seq_lens_first_np = self.seq_lens_first_cpu.numpy()
        self.seq_lens_second_cpu = torch.zeros(self.max_num_reqs,
                                               dtype=torch.int32,
                                               device="cpu",
                                               pin_memory=self.pin_memory)
        self.seq_lens_second_np = self.seq_lens_second_cpu.numpy()
    ...
    def _prepare_inputs(
        ...
        prev, curr = 0, 0
        actual_tokens_in_split_1, actual_tokens_in_split_2 = 0, 0
        for token in num_scheduled_tokens:
            ...
            if use_first:
                ...
                append_2(token)
                actual_tokens_in_split_2 += token
            prev = curr

        num_scheduled_tokens_1 = np.array(tokens_1, dtype=np.int32)
        num_scheduled_tokens_2 = np.array(tokens_2, dtype=np.int32)
        max_num_scheduled_tokens_1 = max(tokens_1)
        max_num_scheduled_tokens_2 = max(tokens_2) if tokens_2 else 0
        num_requests_1 = len(tokens_1)
        ...
        self.query_first_loc_np[1:1 + num_requests_1] = cu_num_tokens_1
        self.query_second_loc_np[1:1 + num_requests_2] = cu_num_tokens_2

        if num_requests_1 + num_requests_2 > num_reqs:
            # split at midpoint
            self.seq_lens_first_np[:num_requests_1] = self.seq_lens_np[:num_requests_1]
            self.seq_lens_first_np[num_requests_1 - 1] -= self.query_second_loc_np[1]

            self.seq_lens_second_np[:num_requests_2] = self.seq_lens_np[num_requests_1 - 1:num_reqs]
        else:
            self.seq_lens_first_np[:num_requests_1] = self.seq_lens_np[:num_requests_1]
        ...
        attn_metadata = self.attn_metadata_builder.build(
            ...
            num_query_1=num_requests_1,
            num_query_2=num_requests_2,
            max_query_len_1=max_num_scheduled_tokens_1,
            max_query_len_2=max_num_scheduled_tokens_2,
            actual_tokens_in_split_1=actual_tokens_in_split_1,
            actual_tokens_in_split_2=actual_tokens_in_split_2,
        )
        # if get_tensor_model_parallel_rank() == 0:
        #     print(f"{num_reqs=}, {total_num_scheduled_tokens=}, {max_num_scheduled_tokens=}, {common_prefix_len=}",
        #           flush=True)
        #     print(f"attn_metadata: {attn_metadata}", flush=True)
    ...
    def execute_model(
        ...
        # if get_tensor_model_parallel_rank() == 0:
        #     print(f"SchedulerOutput: {scheduler_output}", flush=True)
    ...
    def _dummy_run(
        ...
        # if get_tensor_model_parallel_rank() == 0:
        #     print(f"Dummy run hidden_states: {hidden_states}", flush=True)
    ...
    def profile_run(self) -> None:
        ...
        # if get_tensor_model_parallel_rank() == 0:
        #     print(f"Profiled hidden_states: {hidden_states}", flush=True)
        if get_pp_group().is_last_rank:
