...
# ------------------ Configuration ------------------
EVAL_DIR=$1
MASTER_CSV="$EVAL_DIR/figure_qwen3moe_multibatch_summary.csv"
...
NUM_ITERS_WARMUP=10
NUM_GPUS_LIST=(8)
MODEL_NAME_LIST=("Qwen3-235B-A22B")
BASELINE_IMPL_LIST=("baseline_multimem" "baseline_fused" "no_ar")
...
init_benchmark() {
    mkdir -p "$EVAL_DIR"
    echo "model_name,gpu_count,impl,batch_size,avg_latency(ms)" > "$MASTER_CSV"
}
...
extract_latency() {
    ...
    grep "batch size:" "$output_log" | while read -r line; do
...
for model in "${MODEL_NAME_LIST[@]}"; do
    case "$model" in
        "Qwen3-235B-A22B")
            ...
            prefix="pq3m_"
            src_dir="$SCRIPT_DIR/../qwen3moe_src_files"
            dst_file_path="${SCRIPT_DIR}/../../vllm/model_executor/models/qwen3_moe.py"
            extra_args=""
            ;;
        *)
